/* /script.js
   Major 7 vs Dominant 7 Ear Trainer
...
(() => {
  ...
  const RANGES = {
    "hard-3oct": { startOctave: 3, octaves: 3 },
  };
  const LOCKED_ROOT_RANGE = "hard-3oct";

  const CHORD_TYPES = {
    maj7:  { label: "Major 7",    intervals: [0, 4, 7, 11], color: "#1f9d55" },
    dom7:  { label: "Dominant 7", intervals: [0, 4, 7, 10], color: "#ff7a18" }
  };

  // Fixed active chords for this version
  const activeChordKeys = ["maj7", "dom7"];
  ...
  const answerButtonsContainer = $("answerButtonsContainer");
  const refButtonsContainer = $("refButtonsContainer");

  const feedbackOut = $("feedbackOut");
  const miniMount = $("miniMount");

  const compareSection = $("compareSection");
  const compareButtonsContainer = $("compareButtonsContainer");
  ...
  function buildDynamicButtons() {
    answerButtonsContainer.innerHTML = "";
    refButtonsContainer.innerHTML = "";
    compareButtonsContainer.innerHTML = "";

    activeChordKeys.forEach((key) => {
      const typeData = CHORD_TYPES[key];

      // Answer button
      const ansBtn = document.createElement("button");
      ansBtn.type = "button";
      ansBtn.className = "primary answerBtn";
      ansBtn.id = `btn-ans-${key}`;
      ansBtn.style.backgroundColor = typeData.color;
      ansBtn.style.setProperty("--btnPulseRGBA", hexToRgba(typeData.color, 0.3));
      ansBtn.innerHTML = `${typeData.label}`;
      ansBtn.disabled = true;
      ansBtn.addEventListener("click", () => answer(key));
      answerButtonsContainer.appendChild(ansBtn);

      // Ref button
      const refBtn = document.createElement("button");
      refBtn.type = "button";
      refBtn.className = "refBtn";
      refBtn.textContent = typeData.label;
      refBtn.addEventListener("click", () => playReferenceChord(key));
      refButtonsContainer.appendChild(refBtn);

      // Compare button
      const compBtn = document.createElement("button");
      compBtn.type = "button";
      compBtn.className = "refBtn";
      compBtn.id = `btn-comp-${key}`;
      compBtn.textContent = `Play ${typeData.label}`;
      compBtn.addEventListener("click", () => playCompareChord(key));
      compareButtonsContainer.appendChild(compBtn);
    });
  }
  ...
  function stopAllNotesWithUi(fadeSec = 0.05) { stopAllNotes(fadeSec); }
  ...
  async function playReferenceChord(key) {
    ...
    const pitches = CHORD_TYPES[key].intervals.map(interval => root + interval);

    const whenSec = ctx.currentTime + 0.03;
    await playChordWindowed(pitches, whenSec, 2.2, FADE_OUT_SEC);
  }

  async function playCompareChord(key) {
  ...
  function currentMode() { return RANGES[LOCKED_ROOT_RANGE] || RANGES["hard-3oct"]; }
  function activeModeString() { return activeChordKeys.map(k => CHORD_TYPES[k].label).join(", "); }
  ...
  function computeRootBounds() {
    ...
    const c3 = pitchFromPcOct(0, 3); // Lowest allowed root (C3)
    const maxRoot = pitchFromPcOct(1, 5); // Max root C#5 (so highest note is C6)
...
function getDynamicGain(numVoices, dbReductionPerExtraVoice = 2.5) {
  ...
  function pickChord() {
    ...
    const key = activeChordKeys[randomInt(0, activeChordKeys.length - 1)];
    const intervals = CHORD_TYPES[key].intervals;
    
    return {
      qualityKey: key,
      rootPitch,
      pitches: intervals.map(i => rootPitch + i),
    };
  }
  ...
  function displayLongest() { return Math.max(score.longestStored, score.streak); }
  ...
  function updateControls() {
    ...
    // Manage answer button states
    activeChordKeys.forEach(key => {
      const btn = $(`btn-ans-${key}`);
      if (btn) {
        btn.disabled = answerDisabled;
        btn.classList.toggle("answerPulse", !answerDisabled);
      }
    });
  ...
  function computeTwoOctaveWindowForSet(pitches) {
    ...
    // Clamp visual keyboard to C6
    return { lo: clamp(startC, c3, c6), hi: clamp(endC, c3, c6) };
  }
  ...
  function buildMiniKeyboardChord(pitches, highlightSet) {
    ...
    const WHITE_W = 26, WHITE_H = 92, BLACK_W = 16, BLACK_H = 58, BORDER = 8, RADIUS = 14;
    ...
    const innerW = whitePitches.length * WHITE_W;
    ...
    const s = el("svg", {
      ...
      role: "img", "aria-label": "Chord keyboard diagram",
    });
    ...
    style.textContent = `
      ...
      .lbl{ font-family: Arial, sans-serif; font-size:11px; fill: rgba(0,0,0,0.55); font-weight:800; user-select:none; }
      .tone rect{ fill: var(--chordTone) !important; }
      .tone .lbl{ fill: rgba(255,255,255,0.95) !important; }
    `;
    s.appendChild(style);
    s.appendChild(el("rect", { x: BORDER / 2, y: BORDER / 2, width: outerW - BORDER, height: outerH - BORDER, rx: RADIUS, ry: RADIUS, class: "frame" }));

    const gW = el("g"); const gB = el("g");
    s.appendChild(gW); s.appendChild(gB);

    const startX = BORDER; const startY = BORDER;
    ...
    for (let i = 0; i < whitePitches.length; i++) {
      ...
      const name = PC_NAMES_SHARP[pc] + octFromPitch(p);
    ...
    for (let p = lo; p <= hi; p++) {
      ...
      const leftWhitePitch = pitchFromPcOct(leftPc, octFromPitch(p));
      const wi = whiteIndexByPitch.get(leftWhitePitch);
      if (wi == null) continue;

      const x = startX + wi * WHITE_W + WHITE_W - (BLACK_W / 2);
  ...
  async function startNewRound({ autoplay = true } = {}) {
    ...
    if (autoplay) {
      ...
      setFeedback("Identify the 7th chord.");
  ...
  function answer(choiceKey) {
    ...
    const correctKey = chord.qualityKey;
    const isCorrect = choiceKey === correctKey;

    const rootName = noteNameForPc(pcFromPitch(chord.rootPitch));
    const correctLabel = CHORD_TYPES[correctKey].label;
    const choiceLabel = CHORD_TYPES[choiceKey].label;
    ...
    if (isCorrect) {
      ...
      setFeedback(
        `Correct! ✅<br/>` +
        `Chord: <strong>${rootName} ${correctLabel}</strong> — notes ${notesLabel}.`
      );
    } else {
      score.incorrect += 1;
      ...
      setFeedback(
        `Incorrect ❌ (You chose <strong>${choiceLabel}</strong>.)<br/>` +
        `Chord: <strong>${rootName} ${correctLabel}</strong> — notes ${notesLabel} ` +
        `(Answer: <strong>${correctLabel}</strong>).`
      );
    }

    renderScore();
    const pSet = new Set(chord.pitches);
    buildMiniKeyboardChord(chord.pitches, pSet);

    compareSection.classList.remove("hidden");
    activeChordKeys.forEach(k => {
      const b = $(`btn-comp-${k}`);
      if (b) b.textContent = `Play ${rootName} ${CHORD_TYPES[k].label}`;
    });
  ...
  async function downloadScorecardPng(nameInputEl) {
    ...
    ctx.fillText(`Mode: 7th Chords (${activeModeString()})`, W / 2, yCursor);
    ...
    ctx.fillText("7th Chord Ear Trainer - www.eartraininglab.com", W / 2, cardY + cardH - 24);
  ...
  function initTitleSwap() {
    ...
    if (titleImgWide.complete) tryUpdate(); else titleImgWide.addEventListener("load", tryUpdate, { once: true });
    if (titleImgWrapped.complete) tryUpdate(); else titleImgWrapped.addEventListener("load", tryUpdate, { once: true });
  ...
  function bind() {
    ...
    document.addEventListener("keydown", async (e) => {
      ...
      // Arrow key shortcuts for the 2 options
      if (e.code === "ArrowLeft" && canAnswer) { answer("maj7"); return; }
      if (e.code === "ArrowRight" && canAnswer) { answer("dom7"); return; }
